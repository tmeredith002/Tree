...
class TreeGUI:
    def __init__(self, height, leaf, trunk, orn, star, count):
        ...
        tree_width = 2 * self.height + 1
        tree_height = self.height + math.ceil(self.height / 10) + 1
        self.text = tk.Text(self.tree_frame, bg="black", fg="white", font=("Courier", 12), \
                            width=tree_width, height=tree_height) #Snap frame to tree
        self.text.pack()
        #Define tag config colors
        self.text.tag_config("green", foreground="green")
        self.text.tag_config("red", foreground="red")
        self.text.tag_config("blue", foreground="blue")
        self.text.tag_config("gold", foreground="gold")
        self.text.tag_config("saddlebrown", foreground="saddlebrown")

        self.build_tree()

    def build_tree(self):
        # Print the star at the top
        self.text.insert("end", " " * (self.height - 1))
        self.text.insert("end", self.star, "gold")
        self.text.insert("end", "\n")
        # Print the top leaf
        self.text.insert("end", " " * (self.height - 1) + self.leaf, "green")
        self.text.insert("end", "\n")
